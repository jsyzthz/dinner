...
public class TencentSMSSerivce implements ISMSService {
    ...
    public boolean sentSingleMessageByTemplate(String countryId, String phoneNumber, int templateId,
        ...
        try {
            ...
            while (it.hasNext()) {
                String key = it.next();
                paramsArray[index++] = params.get(key);
            }
            SmsSingleSenderResult result =
                ssender.sendWithParam(countryId, phoneNumber, templateId, paramsArray, smsSign, "", ""); // 签名参数未提供或者为空时，会使用默认签名发送短信
            System.out.print(result);
        } catch (HTTPException e) {
            // HTTP响应码错误
            e.printStackTrace();
    ...
    public boolean sentMulitMessageByTemplate(String countryId, ArrayList<String> phoneNumbers, int templateId,
        ...
        try {
            ...
            SmsMultiSenderResult result =
                msender.sendWithParam(countryId, phoneNumbers, templateId, params, smsSign, "", ""); // 签名参数未提供或者为空时，会使用默认签名发送短信
            System.out.print(result);
        } catch (HTTPException e) {
            // HTTP响应码错误
            e.printStackTrace();
